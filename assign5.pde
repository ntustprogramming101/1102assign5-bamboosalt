...
//boolean cIsExist(float[] clockX, float[] cabbageX){
//  for(int i=0; i< cabbageX.length; i++){
...
// boolean cIsExistY(float[] clockY, float[] cabbageY){
//  for(int i=0; i< cabbageY.length; i++){
...
void draw() {
  ...
  case GAME_RUN: // In-Game
    ...
    for (int i = 0; i < soldierX.length; i++) {
      ...
      // Requirement #3: Use boolean isHit(...) to detect collision
      if (isHit(playerX, playerY, 80, 80, soldierX[i], soldierY[i], 80, 80)) {
        playerHealth --;
